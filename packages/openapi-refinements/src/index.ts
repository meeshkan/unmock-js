...
const internalGetRequestBodyFromRef = internalGetComponent(
...
export const changeRefs = (j: Schema): Schema => ({
  ...
  ...(isReference(j.items)
    ...
    ? {
        items: j.items.map(i =>
          isReference(i) ? changeRef(i) : changeRefs(i),
        ),
      }
...
const mapDefaultToCodesFunction = (b: Array<keyof Responses>) => (
  r: Responses,
): Responses =>
  Object.entries(r)
...
) => codesInternal(o, path, operations, mapDefaultToCodesFunction(r));
