...
describe("makeHashable", () => {
  ...
  test("filters correctly with nested object", () => {
    expect(makeHashable({ a: { q: 1, r: 2 }, b: 2, c: 3 }, { a: "q" })).toEqual(
      { a: { r: 2 }, b: 2, c: 3 },
    );
  });
  ...
  test("filters correctly with nested object and array", () => {
    expect(
      makeHashable({ a: { q: 1, r: 2, s: 5 }, b: 2, c: 3 }, { a: ["q", "r"] }),
    ).toEqual({ a: { s: 5 }, b: 2, c: 3 });
  });
  ...
  test("filters something realistic", () => {
    ...
    expect(makeHashable(real, { headers: "w*User-Agentw*" })).toEqual(noAgent);
    expect(
      makeHashable(real, ["story", { headers: "w*User-Agentw*" }]),
